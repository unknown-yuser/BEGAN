...
class BEGANUpdater(training.StandardUpdater):
    def __init__(self, iterator, noise_iterator, optimizer_generator, optimizer_discriminator,
                 generator_lr_decay_interval,
                 discriminator_lr_interval, gamma, k_0, lambda_k, loss_norm, device=-1):
        iterators = {'main': iterator, 'z': noise_iterator}
        optimizers = {
            'gen': optimizer_generator,
            'dis': optimizer_discriminator}

        super(BEGANUpdater, self).__init__(iterators, optimizers, device=device)
        self.gen_lr_decay_interval = generator_lr_decay_interval
        self.dis_lr_decay_interval = discriminator_lr_interval
    ...
    def update_core(self):
        # fake data seed
        z = self.z_batch()
        # generator makes fake data from seeds
        x_fake = self.generator(z)
        # derive fake data's energy by reconstruct fake data throw auto-encoder
        x_fake_recon = self.discriminator(x_fake)
        recon_loss_fake = self.pixel_wise_loss(x_fake, x_fake_recon)

        # derive real data's energy by reconstruct real data throw auto-encoder
        x_real = self.x_batch()
        ...
        # Decay the learning rate of the optimisers by a factor 2
        ...
        reporter.report({'recon_loss_real': recon_loss_real}, self.generator)
        reporter.report({'recon_loss_fake': recon_loss_fake}, self.discriminator)
